...
const ONBOARDING_STEPS: Record<OnboardingStep, StepContent> = {
  ...
  GENERATE: {
    ...
    icon: <Sparkles className="text-accent2" size={24} />,
  ...
  PROCESSING: {
    ...
    icon: <ShieldAlert className="text-warn" size={24} />,
    position: 'center'
  },
  REPORT_OVERVIEW: {
  ...
  WHY_CHANGED: {
    ...
    icon: <BarChart3 className="text-accent2" size={24} />,
...
const Onboarding: React.FC<OnboardingProps> = ({ currentStep, onNext, onPrev, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] pointer-events-none">
      ...
      {isCenter && <div className="absolute inset-0 bg-ink/60 backdrop-blur-sm pointer-events-auto" />}
      
      {/* The Guide Card */}
      <div 
        className="absolute bg-surface rounded-2xl shadow-2xl border border-border2 p-6 pointer-events-auto animate-in fade-in zoom-in duration-300"
        style={getPositionStyles()}
      >
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 text-ink3 hover:text-ink2 transition-colors"
        >
          <X size={20} />
        </button>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-surface2 rounded-xl">
              {stepInfo.icon}
            </div>
            <h3 className="text-lg font-bold text-ink2">{stepInfo.title}</h3>
          </div>
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-border2">
            <div className="flex gap-1">
              {[...Array(9)].map((_, i) => (
                <div 
                  key={i} 
                  className={`h-1.5 w-1.5 rounded-full ${currentIndex === i ? 'bg-accent w-4' : 'bg-surface2'} transition-all`}
                />
              ))}
            </div>
      ...
      {stepInfo.targetId && (
        <div 
          className="absolute ring-[4px] ring-accent ring-offset-4 rounded-xl pointer-events-none transition-all duration-500 ease-in-out"
          style={{
